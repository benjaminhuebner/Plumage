...
# DerivedData/SWBBuildService. The native toolbar button and /plumage-implement
# runs — including runs in other worktrees of the same repo — can all reach
# this script, so guard with a lock keyed on the absolute git common dir
# (identical across worktrees; macOS has no flock). The lock is a file holding
# the owner PID; a lock whose owner is dead is taken over automatically, so a
# kill -9'd gate no longer blocks the next run.
git_common_dir="$(git rev-parse --path-format=absolute --git-common-dir 2>/dev/null || true)"
lock_key=$(printf '%s' "${git_common_dir:-$repo_root}" | shasum 2>/dev/null | cut -c1-12 || echo default)
...
# Acquire by hard-linking a private temp file (content: our PID) to the lock
# path — link(2) is atomic, so the lock can never exist without its owner PID.
# On contention with a live owner, returns 1 with lock_owner set.
lock_owner=""
try_acquire_lock() {
    local tmp="$lock_file.$$"
    printf '%s' "$$" > "$tmp"
...
# Ownership-checked release: never delete a lock another gate has since
