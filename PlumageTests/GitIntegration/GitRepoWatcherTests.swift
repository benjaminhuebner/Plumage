...
struct GitRepoWatcherTests {
    ...
    func burstCoalescesIntoOneChanged() async throws {
        ...
        let watcher = GitRepoWatcher(rawSignals: rawSignals, clock: clock)
...
private actor ChangeCollector {
    ...
    private(set) var last: GitRepoChangeEvent?
    func record(_ event: GitRepoChangeEvent) {
        count += 1
        last = event
    }
}
