...
struct ClaudeUsageResponseTests {
    ...
    func decodesPro() throws {
        ...
        #expect(response.extraSpendingUSD == 0.0)
    ...
    @Test("decodes Max-account fixture with Opus + spending")
    func decodesMax() throws {
        ...
        #expect(response.extraSpendingUSD == 4.27)
    }
    ...
    func decodesEmpty() throws {
        ...
        #expect(response.sevenDayOpus == nil)
        #expect(response.extraSpendingUSD == nil)
    }

    @Test("drops window when pct is missing")
    func dropsWindowWithoutPct() throws {
        let json = #"""
            {
    ...
    func acceptsUnixSeconds() throws {
        let json = #"""
            {
              "five_hour": { "utilization_pct": 10, "resets_at": 1747862400 }
            }
            """#
        let response = try ClaudeUsageResponse.decode(data: Data(json.utf8))
        let resets = try #require(response.fiveHour?.resetsAt)
    ...
    @Test("decodes organization listing")
    func decodesOrgListing() throws {
        let data = try fixture(named: "organizations")
        let orgs = try ClaudeOrganizationListing.decode(data: data)
        #expect(orgs.count == 2)
        #expect(orgs.first?.id == "11111111-2222-3333-4444-555555555555")
        #expect(orgs.first?.name == "Personal")
    }
