...
struct PRMarkdownParserTests {
    ...
    func orderedItems() throws {
        let blocks = PRMarkdownParser.parse("1. first\n2) second")
    ...
    func numberWithoutMarker() {
        for input in ["1.no space", "12345678901. overflow", "1x. nope"] {
            let blocks = PRMarkdownParser.parse(input)
    ...
    func nestedLists() throws {
        let blocks = PRMarkdownParser.parse("- top\n  - nested\n    1. deep")
    ...
    func taskItems() throws {
        let blocks = PRMarkdownParser.parse("- [ ] open\n- [x] done\n- [X] also done")
        try #require(blocks.count == 3)
    ...
    func nonTaskBracket() {
        let blocks = PRMarkdownParser.parse("- [link](https://example.com) text")
    ...
    func tableWithoutDelimiter() {
        let blocks = PRMarkdownParser.parse("| a | b |\n| 1 | 2 |")
        #expect(
            blocks.allSatisfy {
    ...
    private func paragraphText(_ block: PRMarkdownParser.Block) -> AttributedString? {
    ...
    func inlineStyles() throws {
        let blocks = PRMarkdownParser.parse("*em* and **strong** and `code`")
    ...
    func webLinksPreserved() throws {
        let blocks = PRMarkdownParser.parse("[a](https://example.com) [b](http://example.org)")
        let text = try #require(paragraphText(blocks[0]))
    ...
    func unclosedEmphasis() throws {
        let blocks = PRMarkdownParser.parse("*unclosed emphasis")
        let text = try #require(paragraphText(blocks[0]))
    ...
    func inlineInsideBlocks() throws {
        let blocks = PRMarkdownParser.parse("- **bold** item\n\n| [x](javascript:x) |\n|---|")
